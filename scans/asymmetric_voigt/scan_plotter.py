...
def skewed_voigt2(x, amplitude=1.0, center=0.0, sigma=1.0, gamma=None, skew=0.0):
    ...
    return asym * amplitude * voigt(x, 1, center, sigma, gamma=gamma) / voigt(center, 1, center, sigma, gamma=gamma)
...
def txtreader2(filename):
    _,cts,_,_,wvno,_,_,_,_,_,_,_= np.genfromtxt( filename, unpack= True, delimiter=',', skip_header=1)
    return wvno, cts
...
plt.errorbar(freq, counts, yerr=counterr, marker='o', markersize=2, lw = 0, mfc = 'C0', ecolor = 'C0', elinewidth = 0.5, label=finalname.strip('_fit').strip('_feb').strip('_asym_errors').replace('_1step', ' 1$^{st}$ step').replace('_2step', ' 2$^{nd}$ step'))

svm = SkewedVoigtModel()
svm.func = skewed_voigt2
background = ConstantModel()
svm = svm + background
params = svm.make_params(amplitude=933, center=18, sigma=0.13, skew=-0.02, c=50, gamma=27)
